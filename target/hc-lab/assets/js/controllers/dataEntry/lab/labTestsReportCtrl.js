angular.module("labModule").controller("labTestsReportCtrl", ['$scope', '$parse','commonService','labPatientService', '$state','$stateParams','ngDialog','$timeout', function($scope, $parse, commonService,labPatientService,$state, $stateParams,ngDialog,$timeout) {
	...
	function init(){
		...
		if($stateParams.opLabPatientId){
			getLabReport();
		}
		$scope.labTechnicians = [];
	}
	
	function getOperatorList(){
		commonService.getOperatorList($scope.technician).then(function(res){
			$scope.labTechnicians= res;
		})
	}
	
	function getLabReport(){
		$scope.formLoader = true;
		labPatientService.getReport($stateParams.opLabPatientId).then(function(res){
			if(res){
				$scope.labPatientReport = res;
				$scope.opLabPatient = JSON.parse(res.labReport);
				$scope.opLabPatient.createdOn = $scope.labPatientReport.createdOn;
	...
	function updateLabReport(){
		...
		$scope.labPatientReport.labReport = JSON.stringify($scope.opLabPatient);
	...
	function upLoadFile(file,categoryName,testName){
		...
		labPatientService.uploadReport($scope.labPatientReport.id,fd).then(function(res){
			if(res){
				$scope.formLoader = false;
			}else{
				$scope.formLoader = false;
